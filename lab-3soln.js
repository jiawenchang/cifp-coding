...
function calculateCoffeeSize(sizeParameter){ // called a function, parameter in parentheses
    if(sizeParameter === "small") {
       let filteredCoffeeArray =  ml.filter(size => size >= 200 && size < 300); // let = var
       // arrayName.filter, filter acts like cd a loop 
...
function employeeCalculation(ids){
    var amy = 0; // start with number 0 and add from there
    var beth = 0;
    var chad = 0;
    var doug = 0;
    var ed = 0;
    for(var i = 0; i<ids.length; i++){ // i<ids.length means "ids.length" which refers to the length of array is always smaller than its position
        if(ids[i] == 1) { // to select an element in array: arrayName[element]
            amy++; 
        } else if(ids[i] == 2){
            beth++;
        } else if(ids[i] == 3){
